...
| `ALLOW_SUDO=true` | Privileged execution (`ssh_sudo_exec`, `ssh_sudo_run_script`) plus the five sudo-tier path-bearing tools added in v1.4.0: `ssh_sudo_read`, `ssh_sudo_read_redacted`, `ssh_sudo_write`, `ssh_sudo_edit`, `ssh_sudo_sftp_list`. Requires `ALLOW_DANGEROUS_TOOLS=true` too, since sudo tools carry both tags. |
...
### Sudo-tier path-bearing tools (v1.4.0)
